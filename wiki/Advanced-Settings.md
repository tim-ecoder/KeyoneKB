...
## Other entities in keyboard_mechanics.json
...
> For example, the BlackBerry Key2 speed_key (KEYCODE_FUNCTION) is not caught by the keyboard, or the side button "287"

### retraslate-keyboard-meta-key-plus-key-list

#### Relay from the accessibility service to the keyboard: meta_hold + button event (the keyboard treats them like any other events)

#### For cases when the meta_hold event is not passed to the keyboard but is caught by the accessibility service

> For example, the BlackBerry Key2 speed_key meta state ("meta-key-code": "META_FUNCTION_ON") is not caught by the keyboard

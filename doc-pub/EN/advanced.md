...
* Go to KeyoneKb Extended Settings;
* Find your application among the three ADD PLUGIN buttons and click on it — the button will change to ADDED;
...
**How to verify the result (it should work in the application immediately, but if you want to double-check)**
...
3. Take the file plugin\_data.json and place it in /storage/emulated/0/Android/data/com.sateda.keyonekb2/files
...
* But in fact, nothing is deleted from applications already registered in plugin\_data.json files — everything will be restored after restarting the accessibility service;
...
4. JSON files will appear in the folder
   * Letter layouts in files like russian\_hw.json or pocket\_english.json
   * Symbol layouts in files like alt\_hw.json or pocket\_alt\_hw.json
...
1. **Hint** If you want a custom symbol from the symbol layout that will be specific to a letter layout, you can add fields from the symbol layout to the letter layout and they will be picked up and used by default. For example, in the Russian layout, replace the dollar sign with the ruble sign.
...
**"sym-mode-layout" : "symbol" — this is the on-screen symbol panel, dynamically populated with symbols from the alt layout. Do not modify this for now.**
...
> For example, all language layouts (and there are four) for Blackberry Key1-2 share the same symbol layout alt\_hw
...
* Example. Fragment of the alt\_hw.json symbol layout
...
* Important! The last body method must stop further execution ("stop-processing-at-success-result": true)
* Otherwise, all methods from the unconditional block (without meta conditions) will be executed next
...
> No need to get creative here — if it was set (for example, for the ActionSendCharSinglePressNoMeta method) in the default keyboard\_mechanics, it should stay that way
...
* Keycode configuration
  * This is an array — which is iterated through on button press (as soon as something matches, we enter the body)
  * Can be KEYCODEs from [KeyEvent](https://developer.android.com/reference/android/view/KeyEvent)
...
* on-long-press — for a long press > on-hold-xxx and on-long-press are mutually exclusive events, only one should be used
...
#### search-plugins-enabled (search plugins on/off)

#### retranslate-keyboard-key-codes

**For buttons that the keyboard doesn't "catch" but the accessibility service does**

**Such events are relayed from the accessibility service to the keyboard (the keyboard treats them like any other events)**

> For example, the BlackBerry Key2 speed\_key (KEYCODE\_FUNCTION) is not caught by the keyboard, or the side button "287"

#### retraslate-keyboard-meta-key-plus-key-list

**Relay from the accessibility service to the keyboard: meta\_hold + button event (the keyboard treats them like any other events)**

**For cases when the meta\_hold event is not passed to the keyboard but is caught by the accessibility service**

> For example, the BlackBerry Key2 speed\_key meta state ("meta-key-code": "META\_FUNCTION\_ON") is not caught by the keyboard

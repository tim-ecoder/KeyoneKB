...
* Для владельцев **Unihertz Titan Pocket и Slim**. Везде, где указано Ctrl, читать FN; Где указано KEY\_0 - этой кнопки попросту нет.
* Для владельцев **KEY2 НЕ\_РСТ**. где указано Ctrl, читать SPEED\_KEY. Причем, функция SPEED\_KEY для вызова приложения будет продолжать работать (Кроме букв ACXVZ и KEY\_0);
...
5. Shift+Enter двигает курсор в начало абзаца (если мы уже в начале абзаца, то в начало предыдущего абзаца);
6. ALT+DEL удаляет все до предыдущего начала (перевода) строки (абзац);
7. KEY\_0+DEL удаляет слово;
8. ALT+SPACE делает \t (табуляцию в тексте) в режиме редактирования
9. Enter работает как кастомная кнопка (SEARCH, GO, NEXT и пр.), для тех случаев, когда такие опции предлагает хост-приложение (по аналогии как работает BB.Keyboard). Теперь в поиске Wildberries работает нормально.
...
2. Shift+Удержание;
3. В ALT-режиме (2хALT или зажатый ALT) нажатие с удержанием;
4. В ALT-режиме Быстрое нажатие; нажатие-с-удержанием; дает первый символ из списка доп. символов (п.8) (тире на минусе, рубль на долларе)
...
> Из-за особенности реализации ОС Блекбери. _Перед зажатием Нуля, сначала поставить правый палец на клавиатуру (можно начать делать жест)_. Это все-равно весьма удобно, если попривыкнуть, чтобы допустим вернуться на несколько символов назад, подправить, потом вернуться обратно в место набора;

> Теперь чтобы двигать курсор с зажатие KEY\_0 не обязательно сначала ставить палец на клавиатуру, можно сначала зажать KEY\_0 потом начать делать жест;

2. «Постоянный» режим жестов. Включение: 2xKEY\_0. Выключение: 1xKEY\_0. Жесты по клавиатуре влево-вправо будут работать до первого ввода текста.
3. Режим жестов +вверх-вниз (для больших текстов). Включение: 3xKEY\_0; Отключение: 1xKEY\_0. Автоматически отключается после ввода первого символа.

> Важно 3xKEY\_0 включает режим вертикальных жестов, после этого этот режим уже будет работать и по 3xKEY\_0. Пока не будет отключен явно 1xKEY\_0;

> Для Uhihertz **Titan Pocket/Slim** для активации режима жестов по тексту используется 2xFn (механизм "зашит" в ОС)

> **Хинт** _Для листания жестами Telegram надо вывалить курсор из поля ввода стрелкой вверх (sym+u/e)._ Появился Режим перелистывания в режиме редактирования (см. ниже)

5. Двойной клик по клавиатуре (без отпускания второго клика - сразу можно начинать делать жест) активирует режим горизонтальных жестов. Отпускание деактивирует режиме жестов по текстовому полю.
6. Тройной клик по клавиатуре (без отпускания третьего клика - сразу можно начинать делать жест) активирует режим горизонтальных и вертикальных жестов. Отпускание деактивирует режиме жестов по текстовому полю.

> Важно тройной клик включает режим вертикальных жестов, после этого этот режим уже будет работать и по двойному тапу

7. В режиме жестов (в т.ч. вертикальных) по полям ввода можно зажимать SHIFT и выделять текст
...
3. В **Telegram** если начать вводить текст в основном окне, будет поиск по чатам.
4. Можно добавлять поисковые плагины весьма легко ([спец. раздел](https://github.com/tim-ecoder/KeyoneKB/wiki/%D0%A1%D0%BB%D0%BE%D0%B6%D0%BD%D1%8B%D0%B5-%D0%BD%D0%B0%D1%81%D1%82%D1%80%D0%BE%D0%B9%D0%BA%D0%B8)), например уже добавлен плагин для ExDialer
5. К вшитым в приложение плагинам добавлены: Yandex (Maps, Навигатор), Blackberry (Settings, HUB, Notes, Calendar)
6. Поисковый плагин активируется теперь на CTRL+ACXV и на DEL и SPACE. Например, вы скопировали адрес, заходите в Яндекс.Карты и не нажимая поиск нажимаете ctrl+v: поиск откроется сам и туда введется адрес из буфера обмена.

...
> **Внимание.** Невнимательные и неосторожные действия могут привести к неработоспособности клавиатуры, и разобраться, что случилось, не залезая в код, будет весьма сложно.
>
> Например, забыть поставить скобку в JSON (поверьте, на подобные косяки люди тратили десятки часов)
...
* Если вы не обнаружили в списке приложение, которое хотели бы "автоматизировать" в части клика на "лупу", то это достаточно легко можно сделать:
  ...
  2. Перейдите в расширенные настройки K12KB;
  3. Найдите среди трех кнопок ADD PLUGIN свое приложение и нажмите на него — кнопка станет ADDED;
...
Должно сразу работать в приложении, но если хочется еще раз убедиться:
...
> **Важно!** Если удалить приложение, все настроечные файлы удалятся вместе с папкой. Делайте бэкап.
...
Кликер-плагины работают точно так же, как и поисковые плагины, но вместо клика на поиск кликают на поле ввода текста. Удобно для мессенджеров и других приложений, где хочется сразу начать печатать, не тапая по полю ввода.

* Кликер-плагины настраиваются в `plugin_data.json` рядом с поисковыми плагинами
* Механизм добавления и сохранения кликер-плагинов идентичен поисковым плагинам (см. выше)
* Встроенные кликер-плагины: 4PDA, Avito, Kate

> В одном приложении может быть и поисковый плагин, и кликер-плагин — клавиатура кликает в разные поля в зависимости от ситуации.

---

### Кастомизация раскладок клавиатуры

> **Важно!** Если удалить приложение, все кастомные раскладки удалятся вместе с папкой. Делайте бэкап.
...
#### Кастомные символы для раскладки

Если вы хотите кастомный символ из символьной раскладки, который будет специфичен для раскладки. То из символьной раскладки можно добавить поля в буквенную и они подхватятся и будут браться по умолчанию. Например, в русской раскладке на доллар повесить знак рубля.
...
> **Примечание:** `"sym-mode-layout" : "symbol"` — это наэкранная символьная панель, динамически заполняется символами из alt-раскладки. Трогать это пока нельзя.
...
#### Для разных раскладок можно сделать свою механику клавиатуры
...
> **Важно!** Последний метод тела должен быть останавливающим дальнейшее выполнение (`"stop-processing-at-success-result": true`). Иначе следом будут выполняться все методы из безусловного блока (без meta условий).
...
Это массив, который при нажатии кнопки "пробуется" перебором (как только что-то совпало — входим в тело). Могут быть KEYCODE-ы из [KeyEvent](https://developer.android.com/reference/android/view/KeyEvent):
...
> Кастомизация отдельных кнопок: в памяти настройки каждой кнопки из группы размножаются/копируются для каждой кнопки. Это значит, что можно кастомизировать поведение конкретной кнопки (через секцию mechanics или JS-патч), не затрагивая остальные кнопки в той же группе. Полезно, например, когда сломалась какая-то кнопка.
...
* `undo-short-press` — отмена действия одиночного нажатия, когда обнаружено зажатие или двойное нажатие. Используется для мета-кнопок (alt, shift, sym, key0, ctrl), которые теперь срабатывают при нажатии, а не при отпускании.
...
##### Порядок обработки событий (`on-xxx...`)
...
JS-патчи — это движок JavaScript для точечного патчинга JSON-файлов настроек. Позволяет делать небольшие целевые изменения в mechanics, core-настройках или данных плагинов, не заменяя целиком JSON-файлы.

#### Как это работает

* Файлы патчей — JavaScript-скрипты, которые модифицируют объект `json` (разобранный JSON-файл)
* Каждый файл патча привязан к конкретному JSON-файлу по префиксу имени (например, `keyboard_mechanics.2_ctrl_acxvz_with_nav_key.js` патчит `keyboard_mechanics.json`)
* Соглашение об именовании: `<имя_целевого_json>.<номер>_<описание>.js`
* Патчи применяются по порядку после загрузки JSON-файла

#### Пример

Простой патч, меняющий тайминг долгого нажатия на 700мс (`keyboard_core.1_lp700.js`):
...
#### Управление JS-патчами

* JS-патчи можно включать/выключать через меню **Расширенные настройки** — редактирование файлов не требуется
* Пользовательские JS-патчи можно размещать в папке `/storage/emulated/0/K12Kb` рядом с другими файлами кастомизации
* Полезно, чтобы не накатывать каждый раз свои изменения, когда разработчик меняет mechanics
* Можно пробовать на удобство изменения, которые сделали другие пользователи для себя

> JS-патчи — рекомендуемый способ мелких кастомизаций, т.к. они переживают обновления файлов mechanics.

---

### Кастомизация NAV-механизма

NAV-механизм вынесен в JSON и теперь полностью кастомизируется, как и остальная механика клавиатуры. Маппинг NAV-клавиш и поведение можно менять через стандартный подход кастомизации mechanics, описанный выше.

---

### Отображение ошибок JSON

Если JSON-файл не загрузился (например, из-за синтаксической ошибки), ошибка отображается в форме "проверка работы" в Расширенных настройках. Это упрощает отладку пользовательских JSON-правок.

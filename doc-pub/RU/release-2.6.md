...
FI4: Обычно если двигаешь в NAV режиме уже имеющееся выделение с SHIFT то двигается правый ползунок (для blacberry). Чтобы двигать левый ползунок надо сделать 2xSHIFT без отжатия второго раза, тогда будет двигаться левый ползунок и по словам и всеми навигационными кнопками. Для unihertz все иначе - по 1xSHIFT всегда двигается тот ползунок который "поехал" первым, чтобы двинуть "второй", который раньше стоял на месте надо сделать 2xSHIFT без отжатия. Менялся базовый mechanics. Чтобы это заработало надо перенакатить свои кастомизации.
...
2. Либо если п.1 выключен (либо скрол отключился из-за нажатия кнопок) то включается по 2xCTRL. Это будет работать и на unihertz, но не на стоковой ОС, а на более поздних ОС без китайских модификаций ядра
...
BUGFIX: Чуть поменял работу search\_plugins для bb.contacts, bb.dialer;
...
CLICKER\_PLUGINS: Напомню - clicker\_plugin кликает на текстовый ввод, когда заходишь в диалог и начинаешь ввод; ru.fourpda.client, +avito, +kate
...
Кроме того для работы с тормозняком (если он все еще будет мешать) можно увеличивать значение т.н. тайминга срабатывания события удержания. keyboard-core.json -> "time-long-press"
